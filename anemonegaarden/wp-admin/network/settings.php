 ...
 $menu_perms = get_site_option( 'menu_items' ); $menu_items = apply_filters( 'mu_menu_items', array( 'plugins' => __( 'Plugins' ) ) ); if ( $menu_items ) : ?>
			...
			<table id="menu" class="form-table">
				...
				<tr>
					...
					<td>
						<?php
 echo '<fieldset><legend class="screen-reader-text">' . $enable_administration_menus_title . '</legend>'; foreach ( (array) $menu_items as $key => $val ) { echo "<label><input type='checkbox' name='menu_items[" . $key . "]' value='1'" . ( isset( $menu_perms[ $key ] ) ? checked( $menu_perms[ $key ], '1', false ) : '' ) . ' /> ' . esc_html( $val ) . '</label><br/>'; } echo '</fieldset>'; ?>
					</td>
				</tr>
			</table>
			<?php
