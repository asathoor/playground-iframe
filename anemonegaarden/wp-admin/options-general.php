...
<td>
 ...
 wp_enqueue_media(); wp_enqueue_script( 'site-icon' ); $classes_for_upload_button = 'upload-button button-hero button'; $classes_for_update_button = 'button'; $classes_for_wrapper = ''; if ( has_site_icon() ) { $classes_for_wrapper .= ' has-site-icon'; $classes_for_button = $classes_for_update_button; $classes_for_button_on_change = $classes_for_upload_button; } else { $classes_for_wrapper .= ' hidden'; $classes_for_button = $classes_for_upload_button; $classes_for_button_on_change = $classes_for_update_button; } $site_icon_id = (int) get_option( 'site_icon' ); $app_icon_alt_value = ''; $browser_icon_alt_value = ''; $site_icon_url = get_site_icon_url(); if ( $site_icon_id ) { $img_alt = get_post_meta( $site_icon_id, '_wp_attachment_image_alt', true ); $filename = wp_basename( $site_icon_url ); $app_icon_alt_value = sprintf( __( 'App icon preview: The current image has no alternative text. The file name is: %s' ), $filename ); $browser_icon_alt_value = sprintf( __( 'Browser icon preview: The current image has no alternative text. The file name is: %s' ), $filename ); if ( $img_alt ) { $app_icon_alt_value = sprintf( __( 'App icon preview: Current image: %s' ), $img_alt ); $browser_icon_alt_value = sprintf( __( 'Browser icon preview: Current image: %s' ), $img_alt ); } } ?>
	...
	<p class="description">
		<?php
 printf( __( 'The Site Icon is what you see in browser tabs, bookmark bars, and within the WordPress mobile apps. It should be square and at least <code>%1$s by %2$s</code> pixels.' ), 512, 512 ); ?>
	</p>
...
<p class="description" id="home-description">
		<?php
 printf( __( 'Enter the same address here unless you <a href="%s">want your site home page to be different from your WordPress installation directory</a>.' ), __( 'https://developer.wordpress.org/advanced-administration/server/wordpress-in-directory/' ) ); ?>
</p>
<?php endif; ?>
</td>
</tr>
...
<?php
if ( ! is_multisite() ) { $membership_title = __( 'Membership' ); ?>
<tr>
...
<tr>
<?php
$current_offset = get_option( 'gmt_offset' ); $tzstring = get_option( 'timezone_string' ); $check_zone_info = true; if ( str_contains( $tzstring, 'Etc/GMT' ) ) { $tzstring = ''; } if ( empty( $tzstring ) ) { $check_zone_info = false; if ( 0 === (int) $current_offset ) { $tzstring = 'UTC+0'; } elseif ( $current_offset < 0 ) { $tzstring = 'UTC' . $current_offset; } else { $tzstring = 'UTC+' . $current_offset; } } ?>
...
<?php
 global $wp_locale; for ( $day_index = 0; $day_index <= 6; $day_index++ ) : $selected = ( (int) get_option( 'start_of_week' ) === $day_index ) ? 'selected="selected"' : ''; echo "\n\t<option value='" . esc_attr( $day_index ) . "' $selected>" . $wp_locale->get_weekday( $day_index ) . '</option>'; endfor; ?>
</select></td>
</tr>
