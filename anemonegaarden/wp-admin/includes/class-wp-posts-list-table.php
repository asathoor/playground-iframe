				...
				<?php if ( post_type_supports( $screen->post_type, 'page-attributes' ) ) : ?>
					...
					<?php if ( $post_type_object->hierarchical ) : ?>
						...
						<label>
							...
							<?php
 $dropdown_args = array( 'post_type' => $post_type_object->name, 'selected' => $post->post_parent, 'name' => 'post_parent', 'show_option_none' => __( 'Main Page (no parent)' ), 'option_none_value' => 0, 'sort_column' => 'menu_order, post_title', ); if ( $bulk ) { $dropdown_args['show_option_no_change'] = __( '&mdash; No Change &mdash;' ); $dropdown_args['id'] = 'bulk_edit_post_parent'; } $dropdown_args = apply_filters( 'quick_edit_dropdown_pages_args', $dropdown_args, $bulk ); wp_dropdown_pages( $dropdown_args ); ?>
						</label>
				...
				<?php endif; ?>
					...
					<div class="inline-edit-group wp-clearfix">
						...
						<label class="inline-edit-status alignleft">
							...
							<select name="_status">
								<?php
 $inline_edit_statuses = array(); if ( $bulk ) { $inline_edit_statuses['-1'] = __( '&mdash; No Change &mdash;' ); } if ( $can_publish ) { $inline_edit_statuses['publish'] = __( 'Published' ); $inline_edit_statuses['future'] = __( 'Scheduled' ); if ( $bulk ) { $inline_edit_statuses['private'] = __( 'Private' ); } } $inline_edit_statuses['pending'] = __( 'Pending Review' ); $inline_edit_statuses['draft'] = __( 'Draft' ); $inline_edit_statuses = apply_filters( 'quick_edit_statuses', $inline_edit_statuses, $screen->post_type, $bulk, $can_publish ); foreach ( $inline_edit_statuses as $inline_status_value => $inline_status_text ) : ?>
